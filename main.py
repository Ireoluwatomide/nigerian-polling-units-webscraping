...
b.open_headless_chrome_browser(url)
...
for i in range(25, len(len_state)+1):

    polling_unit = {'state': [], 'lga': [], 'ward': [], 'polling_unit_ID': [], 'polling_unit_name': [], 'remark': []}
    
    try:
        b.find_element('xpath://*[@id="statePoll"]').click()
        b.find_element(f'xpath://*[@id="statePoll"]/option[{i}]').click()
        state = b.get_text(f'xpath://*[@id="statePoll"]/option[{i}]')
        ...
        for j in range(2, len(len_lga)+1):
            ...
            b.find_element(f'xpath://*[@id="lgaPoll"]/option[{j}]').click()
            lga = b.get_text(f'xpath://*[@id="lgaPoll"]/option[{j}]')
            ...
            for k in range(2, len(len_ward)+1):
                ...
                b.find_element(f'xpath://*[@id="wardPoll"]/option[{k}]').click()
                ward = b.get_text(f'xpath://*[@id="wardPoll"]/option[{k}]')
                print(f'Extracting data for {ward} ward')
                sleep(1)
                ...
                for l in range(1, len(table)+1):
                    ...
                    polling_unit_ID = b.find_element(f'xpath://*[@id="result"]/tbody/tr[{l}]/td[1]').text
                    polling_unit['polling_unit_ID'].append(polling_unit_ID)

                    polling_unit_name = b.find_element(f'xpath://*[@id="result"]/tbody/tr[{l}]/td[2]').text
                    polling_unit['polling_unit_name'].append(polling_unit_name)

                    remark = b.find_element(f'xpath://*[@id="result"]/tbody/tr[{l}]/td[3]').text
